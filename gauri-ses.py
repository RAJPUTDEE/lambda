...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Send report via SES
        send_report_via_ses(s3_client, report_key, config)
...
def send_report_via_ses(s3_client, report_key, config):
 
    if not config['ses_sender'] or not config['ses_recipient']:
        print("SES sender or recipient not configured. Skipping email.")
        return
    ...
    # Create Email
    msg = MIMEMultipart()
    msg['Subject'] = f"EC2 Inventory Report - {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}"
    msg['From'] = config['ses_sender']
    msg['To'] = config['ses_recipient']
 
    body = MIMEText("Please find attached the EC2 inventory report.", "plain")
    msg.attach(body)
